...
@[reducible, inline] meta def insert_name (nm : name) : ℕ × builder_state := 
--| ⟨nl, n, nms, ait, aiv, rft, rfv⟩ := 
match name_lookup.find nm with 
  | some k := (k, ⟨name_lookup, dcls, names, appears_in_type, appears_in_value, 
                referenced_from_type, referenced_from_value⟩)--(k, mk_empty_builder_state)
  | none :=
    --let bs' : builder_state :=
    (dcls, 
⟨ name_lookup.insert nm dcls,
        dcls+1,
        names.push_back nm,
        appears_in_type.push_back mk_rb_set,
        appears_in_value.push_back mk_rb_set,
        referenced_from_type.push_back mk_rb_set,
        referenced_from_value.push_back mk_rb_set ⟩)
end
...
@[reducible, inline] meta def set_appears_in_type (idx : ℕ) (cset : rb_set ℕ) : builder_state :=
--| ⟨nl, _, nms, ait, aiv, rft, rfv⟩ := ((), 
⟨name_lookup, _, names, appears_in_type.write' idx cset, 
             appears_in_value, referenced_from_type, referenced_from_value ⟩ 
...
meta def collect_consts_aux (e : expr) : (list ℕ) × builder_state :=
e.fold ([]) 
  (λ e' _ st, match e' with
  | expr.const nm _ := let (n, bs) := insert_name bs.name_lookup _ bs.names bs.appears_in_type appears_in_value referenced_from_type referenced_from_value nm in
   (n::st, bs)
  | _ := return st
  end)

/-@[reducible, inline] -/meta def collect_consts (e : expr) : rb_set ℕ :=
let l := collect_consts_aux e in 
rb_set.of_list l 
/-e.mfold (mk_rb_set) 
  (λ e' _ st, match e' with
  | expr.const nm _ := do n ← insert_name nm, return $ st.insert n
  | _ := return st
  end)-/
